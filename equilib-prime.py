...
red_oivt_ico='''

...

...
def weightcalc(stringofelements):
    ...
    for i in range(4):
        ...
        if not (s[0] == ' '):
            weight = weight + elements[(s[0:2]).strip()]*int((s[3:]).strip())
    return weight
...
#creating temp *.ico file from encoded string
icondata= base64.b64decode(red_oivt_ico)
iconfile = open('tempico.ico',"wb")
iconfile.write(icondata)
iconfile.close()
root.iconbitmap('tempico.ico')
os.remove('tempico.ico')
